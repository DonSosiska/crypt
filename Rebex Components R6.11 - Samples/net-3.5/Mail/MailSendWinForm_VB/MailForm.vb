...
#End If
    ...
    Private Sub SendMessageAsync(ByVal message As MailMessage)
        Try
            ...
            ' Start sending... 
            ' the AsyncSendCompletedProxy method will be called when transfer
            ' is finished
            _smtp.BeginSend(message, New AsyncCallback(AddressOf AsyncSendCompletedProxy), Nothing)
        Catch x As Exception
            ' close the connection
            _smtp.Disconnect()
...
#Region "Methods and delagates for handling asynchronous results"
    Delegate Sub FinishedDelegate(ByVal asyncResult As IAsyncResult)


    ''' <summary>
    ''' Because this callback delegate (which is handled by this method) will be triggered
    ''' by the asynchronous Smtp.BeginSend method, it will run in a background thread. Because
    ''' we should only access methods (except Invoke) and properties of any WinForm control
    ''' from the thread that created and owns the control, we must pass
    ''' the event arguments to the <see cref="AsyncSendCompleted"/> method that runs in the
    ''' control's thread. This can be accomplished easily using the Invoke method.
    ''' </summary>
    Private Sub AsyncSendCompletedProxy(ByVal asyncResult As IAsyncResult)
        ' pass the arguments to a method that runs in the thread that owns this control
        Invoke(New FinishedDelegate(AddressOf AsyncSendCompleted), New Object() {asyncResult})
    End Sub 'AsyncSendCompletedProxy


    ''' <summary>
    ''' This method is called through the Invoke method by <see cref="AsyncSendCompletedProxy"/>
    ''' when it receives a SendCompleted event. Therefore, it will always run in the main
    ''' application thread that owns this control and we can safely access properties of its child
    ''' controls.
    ''' </summary>
    Private Sub AsyncSendCompleted(ByVal asyncResult As IAsyncResult)
        Try
